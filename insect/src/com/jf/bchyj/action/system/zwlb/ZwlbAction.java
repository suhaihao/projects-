package com.jf.bchyj.action.system.zwlb;
...
public class ZwlbAction extends InitAction {
	...
	protected void actionQuery(ServletObject so) {
		...
		String sql = "SELECT JFID,JFstatCODE,JFstatNAME,decode(JFSTATE,'0','废弃','1','使用') JFSTATE FROM dzwlb WHERE 1=1 ";
		if(bmbm == null){bmbm="";}
		if(bmmc == null){bmmc="";}
		if(zt == null){zt="";}
		if (bmbm.length()>0) { //
	...
	protected void insert(ServletObject so) {
		...
		String sql_temp="select * from dzwlb where JFstatCODE='"+bmbm+"'";
		int c = so.dc.pjdo.execute(sql_temp);
		if (c > 0) {
			so.httpServletRequest.setAttribute("Message", "作物类别编码已存在！");
            return;
		} 
		String sql = "INSERT INTO dzwlb(JFID,JFstatCODE,JFstatNAME,JFSTATE,JFREMARK) VALUES(SEQU_DZWLB.NEXTVAL,'" 
			+ bmbm + "','" + bmmc + "','"+zt+"','" + bz + "')";
	...
	protected void update(ServletObject servletObject) {
	    ...
	    sql = "update dzwlb set JFstatNAME='"+JFDEPTNAME+"',\n";
	...
	protected void other(ServletObject so) {
		...
		if ("DETAIL".equals(forward)) {
			...
			String sql = "SELECT JFID,JFstatCODE,JFstatNAME,  JFSTATE,JFREMARK FROM dzwlb WHERE jfid='"+jfid+"' ";
