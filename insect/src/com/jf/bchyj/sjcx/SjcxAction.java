...
public class SjcxAction extends InitAction {
	...
	protected void actionQuery(ServletObject so) {
		...
		String sql = "select distinct dbbzb.jfid,dbbzb.jfcode,dbbzb.jftabname,f_get_mc('RYBM',dbbzb.jflister) rymc,to_char(dbbzb.jflistdate,'yyyy-mm-dd') lrrq,decode(dbbzb.jftabsort,'0','系统表','1','普查表','&nbsp;') bbzl from dbbzb,dbbzb_cx where dbbzb.jfstate='1' and dbbzb.jfid=dbbzb_cx.jfdbbzb_id ";
	...
	protected void other(ServletObject so) {
		...
		if ("LIST".equals(forward)) {
			...
			String sql = "select jfid,jfname,f_get_mc('RYBM',jflister) jflister,to_char(jflistdate,'yyyy-mm-dd') jflistdate,decode(jfattribute,'0','公有','1','私有')jfattribute,decode(jfstate,'0','初始','1','提交')jfstate from dbbzb_cx where jfdbbzb_id='" 
				+ dbbzb_jfid + "'";
			...
			ResultSet rs = stmt.executeQuery(sql);
				if (rs.next()) {
				so.httpServletRequest.setAttribute("dbbzb_cx_jfid", rs.getString(1));
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
