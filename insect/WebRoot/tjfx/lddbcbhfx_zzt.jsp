...
<html>
  ...
  while (rsq.next())
  {
    sz="select round(sum(decode(sign(nvl(b."+zd+",0)),1,1,0))*100/count(*),2) from d912_body b,d912_head h where b.jfdyid=h.jfid and b."+zd+">0 and to_char(h.rwsj,'yyyy-mm-dd')>='"+cxsjstr+"' and to_char(h.rwsj,'yyyy-mm-dd')<='"+cxsjend+"'";
  ...
  are=are.substring(0,are.length()-1);
    ...
    %>
     <div style="width: 100%;margin-top: 10px;"><center><font size="5px" color="black"><b>虫率变化分析</b></font></center></div>
   ...
   {
   %>
   <input class="btn" type="button" value="导出Excel" onclick="f_exp('<%=cxsjstr %>','<%=cxsjend %>','<%=ssqyval %>','<%=fxdx %>')"/>&nbsp;
   <%
   }
   %>
   </center>
   ...
   %>
    ...
    function f_exp(cxsjstr,cxsjend,ssqyval,fxdx)
 {
   window.location="/insect/tjfx/lddbcbhfx_zzt.jsp?cxsjstr="+cxsjstr+"&cxsjend="+cxsjend+"&ssqyval="+ssqyval+"&p=exp&fxdx="+fxdx;
 }  
