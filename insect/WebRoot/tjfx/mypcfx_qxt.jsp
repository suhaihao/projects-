...
<html>
  ...
  sij=sij.substring(0,sij.length()-1);
   ...
   <%
   //实际发生线数据
  String sjsj="select round((nvl((sum(b."+zd+")/count(*)),0)),2) from d849_body b,d849_head h where b.jfdyid=h.jfid and to_char(h.rwsj,'yyyy-mm-dd')>='"+cxsjstr+"' and to_char(h.rwsj,'yyyy-mm-dd')<='"+cxsjend+"'";
  ...
  sjfa=sjfa.substring(0,sjfa.length()-1);
   ...
   if(lssj.equals(""))
   {
   lsfs="select round((nvl((sum(b."+zd+")/count(*)),0)),2),to_char(h.rwsj, 'mm-dd') from d849_body b,d849_head h where b.jfdyid=h.jfid and to_char(h.rwsj,'yyyy-mm-dd')>='"+dbsjstr+"' and to_char(h.rwsj,'yyyy-mm-dd')<='"+dbsjend+"'";
   }
   else
   {
  ...
  sjlsfa=sjlsfa.substring(0,sjlsfa.length()-1);
    ...
    <%
    //K线 
     String zdzx="select min(b."+zd+") zx,max(b."+zd+") zd from d849_body b,d849_head h where b.jfdyid=h.jfid and to_char(h.rwsj,'yyyy-mm-dd')>='"+cxsjstr+"' and to_char(h.rwsj,'yyyy-mm-dd')<='"+cxsjend+"'";
  if(ssqyval!=null&&!ssqyval.equals(""))
  {
      if(ssqyval.length()==2)
  {
 ...
 function sjfxt()
 {
 window.open("/insect/tjfx/mypcfx_dqt.jsp?cxsjstr=<%=cxsjstr%>&cxsjend=<%=cxsjend%>&dbsjstr=<%=dbsjstr%>&dbsjend=<%=dbsjend%>&ssqyval=<%=ssqyval%>&fxdx=<%=fxdx%>","","width=1200,height=600")
 }
  function sjfzz()
 {
 window.open("/insect/tjfx/mypcfx_zzt.jsp?cxsjstr=<%=cxsjstr%>&cxsjend=<%=cxsjend%>&ssqyval=<%=ssqyval%>","","width=1200,height=600")
 }
