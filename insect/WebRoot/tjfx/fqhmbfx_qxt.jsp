...
<html>
  ...
  <%
  //赋值X轴时间数据以及标准
  String sql="select to_char(rwsj,'mm-dd') from D903_HEAD where  to_char(rwsj,'yyyy-mm-dd')>='"+cxsjstr+"' and to_char(rwsj,'yyyy-mm-dd')<='"+cxsjend+"' and to_char(rwsj,'yyyy-mm-dd') is not null group by to_char(rwsj, 'mm-dd') order by to_char(rwsj, 'mm-dd')";
  ...
  sij=sij.substring(0,sij.length()-1);
   ...
   <%
   //实际发生线数据
  String sjsj="select round((nvl((sum(b."+zd+")/count(*)),0)),2) from d903_body b,d903_head h where b.jfdyid=h.jfid and b.Y10='0001' and b.Y05='0101' and to_char(h.rwsj,'yyyy-mm-dd')>='"+cxsjstr+"' and to_char(h.rwsj,'yyyy-mm-dd')<='"+cxsjend+"'";
  ...
  sjfa=sjfa.substring(0,sjfa.length()-1);
   ...
   if(lssj.equals(""))
   {
     lsfs="select round((nvl((sum(b."+zd+")/count(*)),0)),2),to_char(h.rwsj, 'mm-dd') from d903_body b,d903_head h where b.jfdyid=h.jfid and b.Y10='0001' and b.Y05='0101' and to_char(h.rwsj,'yyyy-mm-dd')>='"+dbsjstr+"' and to_char(h.rwsj,'yyyy-mm-dd')<='"+dbsjend+"'";
   }
   else
   {
       ...
       lsfs="select round((nvl((sum(b."+zd+")/count(*)),0)),2),to_char(h.rwsj, 'mm-dd') from d903_body b,d903_head h where b.jfdyid=h.jfid and b.Y10='0001' and b.Y05='0101' and to_char(h.rwsj,'yyyy-mm-dd')>='"+dbsjstr+"' and to_char(h.rwsj,'yyyy-mm-dd')<='"+dbsjend+"' and to_char(h.rwsj,'mm-dd')>='"+cxsjstr.substring(4,cxsjstr.length())+"' and to_char(h.rwsj,'mm-dd')<='"+cxsjend.substring(4,cxsjend.length())+"'";
   }
  if(ssqyval!=null&&!ssqyval.equals(""))
  {
    if(ssqyval.length()==2)
  ...
  sjlsfa=sjlsfa.substring(0,sjlsfa.length()-1);
    ...
    <%
    //K线 
     String zdzx="select min(b."+zd+") zx,max(b."+zd+") zd from d903_body b,d903_head h where b.jfdyid=h.jfid and b.Y10='0001' and b.Y05='0101' and  to_char(h.rwsj,'yyyy-mm-dd')>='"+cxsjstr+"' and to_char(h.rwsj,'yyyy-mm-dd')<='"+cxsjend+"'";
  if(ssqyval!=null&&!ssqyval.equals(""))
  {
      if(ssqyval.length()==2)
  {
  ...
  zdzxfa=zdzxfa.substring(0,zdzxfa.length()-1);
   ...
   {
   %>
    <input class="btn" type="button" value="不同地区<%=tit%>变化分析" onclick="sjfxt()"/>&nbsp;
    <input class="btn" type="button" value="病田率变化分析" onclick="sjfzz()"/>&nbsp;
 ...
 {
   window.location="/insect/tjfx/fqhmbfx_qxt.jsp?&cxsjstr="+cxsjstr+"&cxsjend="+cxsjend+"&dbsjstr="+dbsjstr+"&dbsjend="+dbsjend+"&bblx="+bblx+"&lssj="+lssj+"&ssqyval="+ssqyval+"&fxdx="+fxdx+"&p=exp";
 }
 function sjfxt()
 {
 window.open("/insect/tjfx/fqhmbfx_dqt.jsp?cxsjstr=<%=cxsjstr%>&cxsjend=<%=cxsjend%>&dbsjstr=<%=dbsjstr%>&dbsjend=<%=dbsjend%>&ssqyval=<%=ssqyval%>&fxdx=<%=fxdx%>","","width=1200,height=600")
 }
  function sjfzz()
 {
 window.open("/insect/tjfx/fqhmbfx_zzt.jsp?cxsjstr=<%=cxsjstr%>&cxsjend=<%=cxsjend%>&ssqyval=<%=ssqyval%>&fxdx=<%=fxdx%>","","width=1200,height=600")
 }
