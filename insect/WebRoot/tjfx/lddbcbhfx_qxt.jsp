...
<html>
  ...
  else if(fxdx.equals("4"))
  {
  tit="霜霉病2级";
  zd="Y10";
  }
  else if(fxdx.equals("5"))
  {
  tit="霜霉病3级";
  zd="Y11";
  }
  else if(fxdx.equals("6"))
  {
  tit="霜霉病4级";
  zd="Y12";
  }
  ...
  <%
  //赋值X轴时间数据以及标准
  String sql="select to_char(rwsj,'mm-dd') from D912_HEAD where to_char(rwsj,'yyyy-mm-dd')>='"+cxsjstr+"' and to_char(rwsj,'yyyy-mm-dd')<='"+cxsjend+"' and to_char(rwsj,'yyyy-mm-dd') is not null group by to_char(rwsj, 'mm-dd') order by to_char(rwsj, 'mm-dd')";
  ...
  sij=sij.substring(0,sij.length()-1);
   ...
   <%
   //实际发生线数据
  String sjsj="select round((nvl((sum(b."+zd+")/count(*)),0)),2) from d912_body b,d912_head h where b.jfdyid=h.jfid and to_char(h.rwsj,'yyyy-mm-dd')>='"+cxsjstr+"' and to_char(h.rwsj,'yyyy-mm-dd')<='"+cxsjend+"'";
  ...
  sjfa=sjfa.substring(0,sjfa.length()-1);
   ...
   if(lssj.equals(""))
   {
      lsfs="select round((nvl((sum(b."+zd+")/count(*)),0)),2),to_char(h.rwsj, 'mm-dd') from d912_body b,d912_head h where b.jfdyid=h.jfid and to_char(h.rwsj,'yyyy-mm-dd')>='"+dbsjstr+"' and to_char(h.rwsj,'yyyy-mm-dd')<='"+dbsjend+"'";
   }
   else
   {
  ...
  sjlsfa=sjlsfa.substring(0,sjlsfa.length()-1);
    ...
    <%
    //K线 
     String zdzx="select min(b."+zd+") zx,max(b."+zd+") zd from d912_body b,d912_head h where b.jfdyid=h.jfid and to_char(h.rwsj,'yyyy-mm-dd')>='"+cxsjstr+"' and to_char(h.rwsj,'yyyy-mm-dd')<='"+cxsjend+"'";
  if(ssqyval!=null&&!ssqyval.equals(""))
  {
      if(ssqyval.length()==2)
  {
 ...
 {
   window.location="/insect/tjfx/lddbcbhfx_qxt.jsp?&cxsjstr="+cxsjstr+"&cxsjend="+cxsjend+"&dbsjstr="+dbsjstr+"&dbsjend="+dbsjend+"&bblx="+bblx+"&lssj="+lssj+"&ssqyval="+ssqyval+"&fxdx="+fxdx+"&p=exp";
 }
  function sjfzz()
 {
 window.open("/insect/tjfx/lddbcbhfx_zzt.jsp?cxsjstr=<%=cxsjstr%>&cxsjend=<%=cxsjend%>&ssqyval=<%=ssqyval%>&fxdx=<%=fxdx%>","","width=1200,height=600")
 }
