...
<html>
  ...
  <%
  //赋值X轴时间数据以及标准
  String sql="select to_char(rwsj,'mm-dd') from D907_HEAD where  to_char(rwsj,'yyyy-mm-dd')>='"+cxsjstr+"' and to_char(rwsj,'yyyy-mm-dd')<='"+cxsjend+"' and to_char(rwsj,'yyyy-mm-dd') is not null group by to_char(rwsj, 'mm-dd') order by to_char(rwsj, 'mm-dd')";
  ...
  sij=sij.substring(0,sij.length()-1);
   ...
   <%
   //实际发生线数据
  String sjsj="select round((nvl((sum(b."+zd+")/count(*)),0)),2) from d907_body b,d907_head h where b.jfdyid=h.jfid and to_char(h.rwsj,'yyyy-mm-dd')>='"+cxsjstr+"' and to_char(h.rwsj,'yyyy-mm-dd')<='"+cxsjend+"'";
  ...
  sjfa=sjfa.substring(0,sjfa.length()-1);
   ...
   if(lssj.equals(""))
   {
    lsfs="select round((nvl((sum(b."+zd+")/count(*)),0)),2),to_char(h.rwsj, 'mm-dd') from d907_body b,d907_head h where b.jfdyid=h.jfid and to_char(h.rwsj,'yyyy-mm-dd')>='"+dbsjstr+"' and to_char(h.rwsj,'yyyy-mm-dd')<='"+dbsjend+"'";
   }
   else
   {
  ...
  sjlsfa=sjlsfa.substring(0,sjlsfa.length()-1);
    ...
    <%
    //K线 
     String zdzx="select min(b."+zd+") zx,max(b."+zd+") zd from d907_body b,d907_head h where b.jfdyid=h.jfid and to_char(h.rwsj,'yyyy-mm-dd')>='"+cxsjstr+"' and to_char(h.rwsj,'yyyy-mm-dd')<='"+cxsjend+"'";
  if(ssqyval!=null&&!ssqyval.equals(""))
  {
      if(ssqyval.length()==2)
  {
  ...
  function f_exp(cxsjstr,cxsjend,dbsjstr,dbsjend,bblx,lssj,ssqyval,fxdx)
 {
  ...
  function sjfzz()
 {
 window.open("/insect/tjfx/ssscfx_zzt.jsp?cxsjstr=<%=cxsjstr%>&cxsjend=<%=cxsjend%>&ssqyval=<%=ssqyval%>&fxdx=<%=fxdx%>","","width=1200,height=600")
 }
