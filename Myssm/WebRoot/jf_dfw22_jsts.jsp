					...
					<div class="col-lg-12">
						<div class="panel panel-default">
							...
							<div class="panel-body">
								<form action="${path}jsts/sel.do" class="navbar-form"
									style="padding: 0;" method="post">
									<div style="text-align: right;">
										<label>时间： </label> <input id="datetimepickerup"
											name="StarTime" type="text" class="form-control"
											placeholder="起始日期（yyyy-mm-dd）" data-date-format="yyyy-mm-dd"
											value="${StarTime}"> 至 <input id="datetimepickerdon"
											name="EndTime" type="text" class="form-control"
											placeholder="结束日期（yyyy-mm-dd）" data-date-format="yyyy-mm-dd"
											value="${EndTime}"> <label>标题： </label> <input
											name="BT" type="text" class="form-control" value="${BT}">
								...
								<table
									...
									<tbody>
										<jf:forEach var="dfw22" items="${listdfw22}">
											<tr>
												<jf:if test="${dfw22.ZT!=0}">
													<td>${dfw22.RN}</td>
												</jf:if>
												<jf:if test="${dfw22.ZT==0}">
													<td style="color: red;">${dfw22.RN}</td>
												</jf:if>
												<jf:if test="${dfw22.ZT!=0}">
												...
												<jf:if test="${dfw22.ZT==0}">
												<td style="color: red;"><fmt:formatDate value="${dfw22.LRRQ}"
														pattern="yyyy-MM-dd HH:mm:ss" /></td>
												</jf:if>
												<jf:if test="${dfw22.ZT!=0}">
												<td>${dfw22.BT}</td>
												...
												<jf:if test="${dfw22.ZT==0}">
												<td style="color: red;">${dfw22.BT}</td>
												</jf:if>
												<jf:if test="${dfw22.ZT!=0}">
												<td><jf:if test="${dfw22.ZT==0}">未读</jf:if> <jf:if
														test="${dfw22.ZT==1}">已读</jf:if></td>
												</jf:if>
												<jf:if test="${dfw22.ZT==0}">
												<td style="color: red;"><jf:if test="${dfw22.ZT==0}">未读</jf:if> <jf:if
														test="${dfw22.ZT==1}">已读</jf:if></td>
												</jf:if>
												<jf:if test="${dfw22.ZT!=0}">
												<td>${dfw22.LRRY}</td>
												...
												<jf:if test="${dfw22.ZT==0}">
												<td style="color: red;">${dfw22.LRRY}</td>
												</jf:if>
												<td><a href="javascript:void(0);" class="btn btn-info "
													onclick="qxtj('${dfw22.JFID}')"> <i
														class="fa fa-search "></i> 查看
												</a></td>
											</tr>
											<form id="qxuser${dfw22.JFID}"
												action="${path}jsts/gotolabel.do" method="post">
												<input type="hidden" name="JFID" value="${dfw22.JFID}" /> <input
													type="hidden" name="ZT" value="${dfw22.ZT}" />
											</form>
