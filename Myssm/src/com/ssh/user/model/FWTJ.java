package com.ssh.user.model;

public class FWTJ {
	/**
	 * 年
	 */
	private String n;
	/**
	 * 年季
	 */
	private String nj;
	/**
	 * 年月
	 */
	private String ny;
	/**
	 * 日
	 */
	private String r;
	/**
	 * 季度
	 */
	private String jd;
	/**
	 * 月份
	 */
	private String yf;
	/**
	 * 服务内容
	 */
	private String fwnr;
	/**
	 * 项目
	 */
	private String xm;
	/**
	 * 防治负责人
	 */
	private String fzfzrmc;
	/**
	 * 省市
	 */
	private String ss;
	/**
	 * 区县
	 */
	private String qx;
	/**
	 * 乡镇
	 */
	private String xz;
	/**
	 * 用户
	 */
	private String yh;
	/**
	 * 防治面积
	 */
	private Double fzmj;
	/**
	 * 收费金额
	 */
	private Double sfje;
	/**
	 * 亩均收费金额
	 */
	private Double mjje;
	/**
	 * 亩均作业工时
	 */
	private Double mjgs;
	/**
	 * 亩均实际工时*人数
	 */
	private Double mjsjgs;
	/**
	 * 生物农药
	 */
	private Double swny;
	/**
	 * 化学农药
	 */
	private Double hxny;
	/**
	 * 理化诱控
	 */
	private Double lhyk;
	/**
	 * 有益昆虫
	 */
	private Double yykc;
	/**
	 * 其他
	 */
	private Double qt;
	/**
	 * 合计成本
	 */
	private Double hjcb;
	/**
	 * 亩均成本
	 */
	private Double mjcb;
	/**
	 * 亩均净成本
	 */
	private Double mjjcb;
	
	public String getFWNRLB() {
		return FWNRLB;
	}
	public void setFWNRLB(String fWNRLB) {
		FWNRLB = fWNRLB;
	}
	public String getDDJ03_JFID() {
		return DDJ03_JFID;
	}
	public void setDDJ03_JFID(String dDJ03_JFID) {
		DDJ03_JFID = dDJ03_JFID;
	}
	public String getFZFZR() {
		return FZFZR;
	}
	public void setFZFZR(String fZFZR) {
		FZFZR = fZFZR;
	}
	public String getSsbm() {
		return ssbm;
	}
	public void setSsbm(String ssbm) {
		this.ssbm = ssbm;
	}
	public String getQxbm() {
		return qxbm;
	}
	public void setQxbm(String qxbm) {
		this.qxbm = qxbm;
	}
	public String getXzbm() {
		return xzbm;
	}
	public void setXzbm(String xzbm) {
		this.xzbm = xzbm;
	}
	public String getDDJ02_JFID() {
		return DDJ02_JFID;
	}
	public void setDDJ02_JFID(String dDJ02_JFID) {
		DDJ02_JFID = dDJ02_JFID;
	}
	private String FWNRLB;
	private String DDJ03_JFID;
	private String FZFZR;
	private String ssbm;
	private String qxbm;
	private String xzbm;
	private String DDJ02_JFID;
	public String getN() {
		return n;
	}
	public void setN(String n) {
		this.n = n;
	}
	public String getNj() {
		return nj;
	}
	public void setNj(String nj) {
		this.nj = nj;
	}
	public String getNy() {
		return ny;
	}
	public void setNy(String ny) {
		this.ny = ny;
	}
	public String getR() {
		return r;
	}
	public void setR(String r) {
		this.r = r;
	}
	public String getJd() {
		return jd;
	}
	public void setJd(String jd) {
		this.jd = jd;
	}
	public String getYf() {
		return yf;
	}
	public void setYf(String yf) {
		this.yf = yf;
	}
	public String getFwnr() {
		return fwnr;
	}
	public void setFwnr(String fwnr) {
		this.fwnr = fwnr;
	}
	public String getXm() {
		return xm;
	}
	public void setXm(String xm) {
		this.xm = xm;
	}
	
	public String getFzfzrmc() {
		return fzfzrmc;
	}
	public void setFzfzrmc(String fzfzrmc) {
		this.fzfzrmc = fzfzrmc;
	}
	public String getSs() {
		return ss;
	}
	public void setSs(String ss) {
		this.ss = ss;
	}
	public String getQx() {
		return qx;
	}
	public void setQx(String qx) {
		this.qx = qx;
	}
	public String getXz() {
		return xz;
	}
	public void setXz(String xz) {
		this.xz = xz;
	}
	public String getYh() {
		return yh;
	}
	public void setYh(String yh) {
		this.yh = yh;
	}
	public Double getFzmj() {
		return fzmj;
	}
	public void setFzmj(Double fzmj) {
		this.fzmj = fzmj;
	}
	public Double getSfje() {
		return sfje;
	}
	public void setSfje(Double sfje) {
		this.sfje = sfje;
	}
	public Double getMjje() {
		return mjje;
	}
	public void setMjje(Double mjje) {
		this.mjje = mjje;
	}
	public Double getMjgs() {
		return mjgs;
	}
	public void setMjgs(Double mjgs) {
		this.mjgs = mjgs;
	}
	public Double getMjsjgs() {
		return mjsjgs;
	}
	public void setMjsjgs(Double mjsjgs) {
		this.mjsjgs = mjsjgs;
	}
	public Double getSwny() {
		return swny;
	}
	public void setSwny(Double swny) {
		this.swny = swny;
	}
	public Double getHxny() {
		return hxny;
	}
	public void setHxny(Double hxny) {
		this.hxny = hxny;
	}
	public Double getLhyk() {
		return lhyk;
	}
	public void setLhyk(Double lhyk) {
		this.lhyk = lhyk;
	}
	public Double getYykc() {
		return yykc;
	}
	public void setYykc(Double yykc) {
		this.yykc = yykc;
	}
	public Double getQt() {
		return qt;
	}
	public void setQt(Double qt) {
		this.qt = qt;
	}
	public Double getHjcb() {
		return hjcb;
	}
	public void setHjcb(Double hjcb) {
		this.hjcb = hjcb;
	}
	public Double getMjcb() {
		return mjcb;
	}
	public void setMjcb(Double mjcb) {
		this.mjcb = mjcb;
	}
	public Double getMjjcb() {
		return mjjcb;
	}
	public void setMjjcb(Double mjjcb) {
		this.mjjcb = mjjcb;
	}
	
}
