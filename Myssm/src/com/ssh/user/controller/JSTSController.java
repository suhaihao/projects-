...
public class JSTSController implements ServletConfigAware, ServletContextAware {
		...
		if (DDJ01_JFIDS == null) {
			...
			try {
				...
				if (mySmartUpload.getRequest().getParameter("ZT").equals("2")) {
					return "redirect:/jsts/gotolabel.do";
				}
	...
	public String upjsr(HttpServletRequest request, HttpServletResponse response) {
		...
		String DDJ01_JFID=user.getDDJ01_JFID();
		DFW22_JSR dFW22_JSR=new DFW22_JSR();
		dFW22_JSR.setDDJ01_JFID(DDJ01_JFID);
		dFW22_JSR.setDFW22_JFID(DFW02_JFID);
		dFW22_JSR.setZT("1");
		dFW22Service.upjsr(dFW22_JSR);
		return "redirect:/jsts/sel.do";
	}
	/**
	 * 已读登陆更新
